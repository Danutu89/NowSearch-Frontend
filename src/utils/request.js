...
const jsonRequest = {
    ...
    request(method, url, data, options) {
       ...
       const toJson = (response) => {
          ...
          if (jsonRequest.logger)
             jsonRequest.logger(new Date().toISOString(), 'response', settings.method,
    ...
    get:    (url, params,   options) => jsonRequest.request('GET',    url, params,   options),
    post:   (url, resource, options) => jsonRequest.request('POST',   url, resource, options),
    put:    (url, resource, options) => jsonRequest.request('PUT',    url, resource, options),
    patch:  (url, resource, options) => jsonRequest.request('PATCH',  url, resource, options),
    delete: (url, resource, options) => jsonRequest.request('DELETE', url, resource, options),
